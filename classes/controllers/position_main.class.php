...
class MainController extends ApplicationController {
    ...
    function MainController() {
        ...
        $this->authorize_before_exec['mod'] = ROLE_STAFF;
        ...
        $this->breadcrumb = array('Stocks' => '/stocks', 'Positions' => '/positions');
    }
/*
  ...
  */   
    private function filter_save($filter) {
        $_SESSION['current_page']['position']['filter'] = $filter;
    }
    ...
    function index() 
    {
        ...
        //debug("1682", $_REQUEST);
        //print_r($_SESSION['current_page']['position']['filter']);
        if (isset($_REQUEST['btn_setfilter'])) {
            ...
            $steelgrades = '';
            if (isset($form['steelgrade'])) {

                foreach ($form['steelgrade'] as $key => $steelgrade_id) {
                    $steelgrade_id = Request::GetInteger($key, $form['steelgrade']);
                    if ($steelgrade_id <= 0) {
                        continue;
                    }

                    $steelgrades = $steelgrades . (empty($steelgrades) ? '' : ',') . $steelgrade_id;
                }
            }

            $stockholders = '';
            if (isset($form['stockholder'])) {
                foreach ($form['stockholder'] as $key => $stockholder_id) {
                    $stockholder_id = Request::GetInteger($key, $form['stockholder']);
                    if ($stockholder_id <= 0) {
                        continue;
                    }

                    $stockholders = $stockholders . (empty($stockholders) ? '' : ',') . $stockholder_id;
                }
            }
            ...
            //разбиваем строку $thickness по забятой. $thickness теперь массив значений
            if ($_SESSION['user']['id'] == '1682') {
                $thickness = Request::GetStringArray('thickness', $form);
            ...
            $keyword = Request::GetString('keyword', $form, '', 200, false);
            //debug('1671', $keyword);
//            $notes          = Request::GetString('notes',           $form);
            ...
            if ($_SESSION['user']['id'] == '1682') {
                //в форме показываем список введенных ранее значений thickness
                $thickness_view = implode(",", $thickness);
                $filter = (empty($stock_id) ? '' : 'stock:' . $stock_id . ';')
                        . (empty($locations) ? '' : 'location:' . $locations . ';')
                        . (empty($stockholders) ? '' : 'stockholder:' . $stockholders . ';')
                        . (empty($deliverytimes) ? '' : 'deliverytime:' . $deliverytimes . ';')
                        . (empty($steelgrades) ? '' : 'steelgrade:' . $steelgrades . ';')
                        . (empty($thickness) ? '' : 'thickness:' . $thickness_view . ';')
...
//                        . (empty($notes) ? '' : 'notes:' . $notes . ';')
        ...
        //
        if (empty($filter)) {
            $this->page_name = 'Steel Plates';
        } else {
        //по просьбе Мики убрал (filtered)
            $this->page_name = 'Steel Plates';
        ...
        if (!empty($target_doc)) {
            if ($target_doc == 'order' || $target_doc == 'neworder') {
                $this->page_name = 'Positions for Order';
            }
            if ($target_doc == 'ra') {
                $this->page_name = 'Positions for Release Advice';
            }
        }
...
//        $notes                  = Request::GetString('notes', $filter_params);
        ...
        if ($stock_id <= 0) {
            $stock_id = 1; //если склад не указан то ищет европейский
        }
        if (!empty($revision)) {
...
//        $this->_assign('notes',         $notes);
        ...
        if ($stock_id > 0) {
            $stocks = new Stock();
            $this->_assign('steelgrade_list', $steelgrades);
        }
        ...
        //debug("1682", $_REQUEST);
        $this->_display('index');  
        /*if ($_SESSION['user']['id'] == '1682' || $_SESSION['user']['id'] == '1705') {
            $this->_display('index');  
        } else {
            $this->rcontext = true;
     ...
     */
    function add() {
        $stock_id = Request::GetInteger('id', $_REQUEST);   //приходит из js

        $biz_id = 0;
        $supplier_id = 0;

        if (isset($_REQUEST['btn_save'])) { //получаем данные формы
            $form = $_REQUEST['form'];
            $positions = isset($_REQUEST['positions']) ? $_REQUEST['positions'] : array();
            //удаляю позиции где не указана стоимость
            ...
            if ($okay_flag) {
                ...
                foreach ($positions as $key => $row) {
                    ...
                    $modelSteelPosition->Add(0, $stock_id, $product_id, $biz_id, $location_id, $producer_id, $stock['dimension_unit'], $stock['weight_unit'], $stock['price_unit'], $stock['currency'], $steelgrade_id, $thickness, $width, $length, $unitweight, $qtty, $weight, $price, $value, $delivery_time, $notes, $internal_notes);
                }
    ...
    function edit() {
        ...
        if (isset($_REQUEST['btn_cancel'])) {
            ...
            foreach ($items as $item) {
                SteelItem::Unlock($item['steelitem']['id']);
            }
            ...
            if (!isset($_REQUEST['item_property'])) {
                _404();
            }
            ...
            //проверяем чтоб location_id каждого итема были одинаковые.
            //
            //@version 20140529, uskov
            $counter = 0;   //считаем итемы
            foreach ($_REQUEST['item'] as $item) {
                $location_id = Request::GetInteger('location_id', $item);
                $counter ++;
                if ($counter > 1 && $location_id !== $prev){
                    $this->_message("In one position you can have only one location!", MESSAGE_ERROR);
                    $this->_redirect(array('position', $_REQUEST['id'], 'edit'));
                    $no_errors = false;
                }
                $prev = $location_id;
            }
            ...
            if ($no_errors) {
                ...
                foreach ($_REQUEST['item'] as $key => $item) {
                    ...
                    // сохраняет айтем                        
                    $result = $items->Save($item_id, $position_id, $guid, $product_id, $biz_id, $location_id, $dimension_unit, $weight_unit, $price_unit, $currency, $steelgrade_id, $thickness, $thickness_measured, $width, $width_measured, $width_max, $length, $length_measured, $length_max, $unitweight, $price, $unitweight * $price, $delivery_time, $notes, $internal_notes, $supplier_id, $supplier_invoice_no, $supplier_invoice_date, $purchase_price, 0, $in_ddt_number, $in_ddt_date, $ddt_number, $ddt_date, $owner_id, $status_id, $is_virtual, $mill, $system, $unitweight_measured, $unitweight_weighed, $current_cost, $pl, $load_ready, $purchase_currency, $in_ddt_company_id, $ddt_company_id, $nominal_thickness_mm, $nominal_width_mm, $nominal_length_mm, $is_ce_mark, $is_mec_prop_not_required);
                    ...
                    if (empty($result) || isset($result['ErrorCode'])) {
                        ...
                        $result = $items->SaveProperties($result['id'], $heat_lot, $c, $si, $mn, $p, $s, $cr, $ni, $cu, $al, $mo, $nb, $v, $n, $ti, $sn, $b, $ceq, $tensile_sample_direction, $tensile_strength, $yeild_point, $elongation, $reduction_of_area, $test_temp, $impact_strength, $hardness, $ust, $sample_direction, $stress_relieving_temp, $heating_rate_per_hour, $holding_time, $cooling_down_rate, $condition, $normalizing_temp);
                ...
                if ($no_errors) {
                    ...
                    if (!$position['steelposition']['inuse']) {
                        ...
                        $result = $positions->Save($position_id, $stock_id, $product_id, $biz_id, 0, 0, $dimension_unit, $weight_unit, $price_unit, $currency, $steelgrade_id, $thickness, $width, $length, $unitweight, $qtty, $weight, $price, $value, $delivery_time, $notes, $internal_notes);
        ...
        foreach ($items as $row) {
            if ((isset($row['steelitem']['order_id']) && $row['steelitem']['order_id'] > 0) || (isset($row['steelitem']['is_eternal']) && $row['steelitem']['is_eternal'])) {
                $eternal_qtty++;
            }
        }

        if (count($items) == $eternal_qtty)
            $this->_assign('items_eternal', true);
    ...
    function groupedit() {
        ...
        if (isset($_REQUEST['btn_cancel'])) {
            // освобождает позиции
            foreach ($list as $row)
                SteelPosition::Unlock($row['steelposition_id']);
            ...
            foreach ($_REQUEST['position_id'] as $key => $position_id) {
                ...
                if (!empty($clear_notes))
                    $notes = '';
                else if (!empty($group_notes))
                    $notes = $group_notes;
                if (!empty($clear_internal_notes))
                    $internal_notes = '';
                else if (!empty($group_internal_notes))
                    $internal_notes = $group_internal_notes;
                if (!empty($clear_delivery_time))
                    $delivery_time = '';
                else if (!empty($group_delivery_time))
                    $delivery_time = $group_delivery_time;

                $result = $positions->Save($position_id, $stock_id, 0, $biz_id, 0, $supplier_id, $stock['dimension_unit'], $stock['weight_unit'], $stock['price_unit'], $stock['currency'], $steelgrade_id, $thickness, $width, $length, $unitweight, 0, $weight, $price, $value, $delivery_time, $notes, $internal_notes);
    ...
    function reserved() {
        if (isset($_REQUEST['btn_remove'])) {
            if (isset($_REQUEST['reserv'])) {
                $positions = new SteelPosition();
                foreach ($_REQUEST['reserv'] as $id => $value)
                    $positions->ReserveRemove($id);
