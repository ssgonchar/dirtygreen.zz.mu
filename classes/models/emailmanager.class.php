...
//_epd($rowset);
     ...
     */
    public function GetList($object_alias, $object_id, $mailbox_id, $type_id, $doc_type_id, $is_deleted, $keyword, $approve_by, $page_no = 0, $per_page = ITEMS_PER_PAGE, $sorter)
    {
        ...
        $hash       =   'emails-filter-' . md5('object_alias-' . $object_alias . '-object_id-' . $object_id . '-mailbox-' . $mailbox_id .
                        '-type-' . $type_id . '-doctypeid-' . $doc_type_id . '-is_deleted-' . $is_deleted . '-keyword-' . $keyword . '-approve_by-' . $approve_by . '-page-' . $page_no . '-count-' . $per_page . '-sorter-' . $sorter);
        
        if (empty($keyword))
        {
            $result = $this->_get_list($object_alias, $object_id, $mailbox_id, $type_id, $doc_type_id, $is_deleted, $approve_by, $hash, $start, $per_page);
        }
        else
        {
...
//TODO: реализовать фильтрацию по doc_type_id
             ...
             $result = $this->_search($object_alias, $object_id, $mailbox_id, $type_id, $keyword, $hash, $start, $per_page);
        }
        
        //dg($result);
        if(!empty($sorter)) {
                usort($result, array('EmailManager', "cmp_".$sorter));
        }
    ...
    function FillEmailInfo($rowset, $id_fieldname = 'email_id', $entityname = 'email', $cache_prefix = 'email')
    {
        ...
        foreach ($rowset as $key => $row) 
        {
            ...
            if (isset($row[$entityname]['objects']))
            {
                ...
                // закомментил т.к. переписывает драйвера и навигатора даже если у письма он указан
...
//        $attachments        = $modelAttachment->FillAttachmentInfo($data_set);
    ...
    public function GetLastBizes()
    {
        ...
        $rowset     = $this->_get_cached_data($hash, 'sp_email_get_last_bizes_new', array($this->user_id), $cache_tags);
   ...
   function close_tags($html)
     ...
     */
    function SearchCompany($search_string, $country_id, $region_id, $city_id, $industry_id, $activity_id, $speciality_id, $product_id, $feedstock_id, $relation, $status, $page_no = 0, $per_page = ITEMS_PER_PAGE)
    {
        $page_no    = $page_no > 0 ? $page_no : 1;
        $per_page   = $per_page < 1 ? ITEMS_PER_PAGE : $per_page;
        $start      = ($page_no - 1) * $per_page;
    ...
    /**
     * Сохраняет список получателей письма в БД
     * 
     * @author SU
     */
    /*public function SaveRecipients($email_id)
    {
        $emails = array();
        $emails = $_SESSION['emails_array'];
        $modelEmail = new Email();
        //если у письма уже есть получатели - удаляю их из БД
        $query  = '';
        ...
        foreach ($emails as $key => $value) {
            ...
            //сохраняю получателей в БД
            $query .= "INSERT INTO `email_recipients` (`company_id`, `email_adress`, `created_by`, `email_id`)";
            $query .= " VALUES ('{$company_id}', '{$email_adress}', '{$user_id}', '{$email_id}')";
            $result = $modelEmail->table->_exec_raw_query($query);
        }
    }*/
    
    /**
     * Возвращает список получателей письма из БД (табл. email_recipients)
     * @param int $email_id id письма
     * 
     * @author SU
     */
    public function GetRecipients($email_id)
    {
        $modelEmail = new Email();
        ...
        $query .= "WHERE object_id = '{$email_id}'";
    ...
    /**
     * Сохраняет в табл. email_objects id вложенных документов (shared docs)
     * 
     * @author SU
     */
    public function SaveSharedDocs($email_id)
    {
        $doc_ids = $_SESSION['doc_ids'];
        $modelEmail = new Email();
        //если у письма уже есть документы - удаляю их из БД
        $query  = '';
        $query .= "DELETE FROM `email_objects` ";
        $query .= "WHERE email_id = '{$email_id}' ";
        $query .= "AND object_alias = 'attached_doc'";
        $result = $modelEmail->table->_exec_raw_query($query);
        
        foreach ($doc_ids as $key => $value) {
            $query         = '';
            $object_alias  = 'attached_doc';
            $object_id     = $value;
            $user_id       = $this->user_id;
            //сохраняю ids  в БД
            $query .= "INSERT INTO `email_objects` (`email_id`, `object_alias`, `object_id`, `created_by`) ";
            $query .= "VALUES ('{$email_id}', '{$object_alias}', '{$object_id}', '{$user_id}')";
            $result = $modelEmail->table->_exec_raw_query($query);
        }
    }
    
    /**
     * Возвращает список вложенных документов к письму (shared_docs)
     * 
     * @author SU
     */
    public function GetSharedDocs($email_id)
    {
        //запрос в БД: табл. email_objects
        $query  = "SELECT * FROM email_objects WHERE email_id = '{$email_id}' ";
        $query .= "AND object_alias = 'attached_doc'";
        $resource = $this->table->_exec_raw_query($query);
        $result   = $this->table->_fetch_array($resource);
        
        //получаю массив аналогичный получаемому массиву при поиске shared_docs
        $shared_docs = Array();
        $modelAttachment = new Attachment();
        foreach ($result as $row) {
            $shared_docs['data'][]['attachment'] = $modelAttachment->GetById($row['object_id']);
        }
        $shared_docs['count'] = count($shared_docs['data']);
        //debug('1682', $shared_docs);
        return $shared_docs;
    }
    
    private static function cmp_sender($a, $b) {
        return strcmp($a['email']['recipient_address'], $b['email']['recipient_address']);
    }    
    private static function cmp_subject($a, $b) {
        return strcmp($a['email']['subject'], $b['email']['subject']);
    }    
    private static function cmp_date($a, $b) {
        return strcmp($a['email']['date_mail'], $b['email']['date_mail']);
    }
    
    /**
     * При отправке нового письма пересохраняет object_alias, object_id для получателей, добавленных из системы
     * 
     * @param string $email_id ID нового письма
     * 
     * @author SU
     */
    public function UpdateRecipientsForSend($email_id)
    {
        $user_id = $this->user_id;
        
        $query  = '';
        $query  = "UPDATE `email_recipients` ";
        $query .= "SET object_alias = 'email', ";
        $query .= "object_id = '{$email_id}' ";
        $query .= "WHERE object_alias = 'newemail' ";
        $query .= "AND object_id = '{$user_id}'";
                
        $result = $this->table->_exec_raw_query($query);
        //return $result;
    }
}