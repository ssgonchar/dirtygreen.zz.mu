...
class SteelItem extends Model
{
    ...
    */
    function Remove($id)
    {
        ...
        //Uskov. 
        //sp_steelitem_mod_remove не удаляет итем только если:
        // (in_ddt_id > 0 OR supplier_invoice_id > 0 OR status_id >= ITEM_STATUS_RELEASED)
        $result = $this->CallStoredProcedure('sp_steelitem_mod_remove', array($this->user_id, $id));
        $result = isset($result) && isset($result[0]) ? $result[0] : array();
...
*/        
    ...
    /**
     * Обновляет location итемов которые в RA если у них указан $dest_stockholder_id
     * 
     * @param mixed $item_id
     * @param mixed $dest_stockholder_id
     * @param mixed $location_id
     ...
     * @Uskov
     */
    function ItemLocationUpdate($item_id, $dest_stockholder_id, $location_id, $status_id)
    {
        $this->CallStoredProcedure('sp_item_location_update', array($this->user_id, $item_id, $dest_stockholder_id, $location_id, $status_id));
