...
class Biz extends Model
{
    ...
    {
		//print_r($parent_id);
		if($parent_id == '6971') $parent_id='2376';
...
//$rowset = null;
    ...
    function SaveNavigators($biz_id, $navigators)
    {
        ...
        foreach($this->GetNavigators($biz_id, false) as $row)
        {
            ...
            if (isset($navigators[$user_id]) 
                && ((isset($navigators[$user_id]['is_driver']) && !empty($row['is_driver'])) || (!isset($navigators[$user_id]['is_driver']) && empty($row['is_driver'])))
            )
            {
    ...
    */
    ...
    function GetCompanies($biz_id, $role = '')
    {
        ...
        {
            foreach ($rowset as $key => $row) if ($row['role'] != $role) unset($rowset[$key]);
        }
    ...
    function FillMainBizInfo($rowset, $id_fieldname = 'biz_id', $entityname = 'biz', $cache_prefix = 'biz')
    {
        ...
        foreach ($rowset as $key => $row)
        {
            ...
            if ($row['status'] == 'idea') $rowset[$key][$entityname]['status_title'] = 'Idea';
            if ($row['status'] == 'marketing') $rowset[$key][$entityname]['status_title'] = 'Marketing';
            if ($row['status'] == 'negotiation') $rowset[$key][$entityname]['status_title'] = 'Negotiation';
            if ($row['status'] == 'admin') $rowset[$key][$entityname]['status_title'] = 'Contract Administration';
            if ($row['status'] == 'closed') $rowset[$key][$entityname]['status_title'] = 'Contracted & Closed';
            if ($row['status'] == 'repeat') $rowset[$key][$entityname]['status_title'] = 'Contracted & Repeat Negotiation';
            if ($row['status'] == 'suspended') $rowset[$key][$entityname]['status_title'] = 'Suspended';
            if ($row['status'] == 'abandoned') $rowset[$key][$entityname]['status_title'] = 'Abandoned';
            if ($row['status'] == 'concluded') $rowset[$key][$entityname]['status_title'] = 'Concluded';
