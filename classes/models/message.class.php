...
class Message extends Model
{
    ...
    {
        $hash       = 'messages-obj-' . $object_alias . '-objid-' . $object_id . '-count';
       // $object_id = 'zzzzzz';
		$cache_tags = array('messages', 'messages-obj-' . $object_alias . '-objid-' . $object_id);
...
*/
        ...
        Message::SetChatLastMessageId($message_id);
    ...
    function SaveObject($message_id, $type_id, $role_id, $sender_id, $object_alias, $object_id)
    {
        ...
        Message::SetChatLastMessageId($message_id, $object_alias, $object_id);
    ...
    private function parse_content($content, $encode_content = true)
    {
        ...
        if (!empty($matches))
        {
            ...
            {
                $content = str_replace('&lt;ref message_id=' . $ref_id . '&gt;', '<a clas="btn btn-success btn-xs" href="javascript: void(0);" onclick="show_chat_message(' . $ref_id . ');">', $content);
        ...
        if (!empty($matches))
        {
            foreach ($matches[1] as $key => $ref_id)
            {
                $content = str_replace('&lt;ref email_id=' . $ref_id . '&gt;', '<a clas="btn btn-success btn-xs" href="javascript: void(0);" onclick="show_email_message(' . $ref_id . ');">', $content);
                $content = str_replace('&lt;/ref&gt;', '</a>', $content);
            }
        }
