...
class Order extends Model {
    ...
    function PositionAddFromStock($order_id, $position_id, $qtty) {
        ...
        $result1 = $this->SavePosition($order_id, $position_id, $result['biz_id'], $result['dimension_unit'], $result['weight_unit'], $result['price_unit'], $result['currency'], $result['steelgrade_id'], $result['thickness'], $result['width'], $result['length'], $result['unitweight'], $result['qtty'], $result['weight'], $result['price'], $result['value'], $result['deliverytime'], $result['internal_notes'], $result['order_status']);
    ...
    function Remove($order_id) {
        ...
        foreach ($positions as $position)
            $this->RemovePosition($order_id, $position['position_id']);
     ...
     * @param mixed $keyword
     */
    function GetList($order_for, $biz_id, $company_id, $period_from, $period_to, $status, $steelgrade_id, $thickness, $width, $keyword, $type, $page_no = 0, $per_page = ITEMS_PER_PAGE) {
    ...
    function GetLastOrderId() {
        $last_order_id = $this->CallStoredProcedure('sp_order_get_last_id', array($this->user_id));
        //$last_order_id = array("order" => "4333");
    ...
    function GetPositions($order_id) {
        ...
        foreach ($rowset as $key => $row) {
            ...
            foreach ($steelitems as $steelitem) {
                ...
                // locations
                if (isset($steelitem['location']) && isset($steelitem['location']['title']) && !empty($steelitem['location']['title']) && !isset($row['location'][$steelitem['location']['title']])) {
     ...
     */
    function SavePosition($order_id, $position_id, $biz_id, $dimension_unit, $weight_unit, $price_unit, $currency, $steelgrade_id, $thickness, $width, $length, $unitweight, $qtty, $weight, $price, $value, $deliverytime, $internal_notes, $order_status) {
        // создает новые позиции
        if (empty($position_id)) {
            $steelpositions = new SteelPosition();
            $result = $steelpositions->Add(0, 0, 92, $biz_id, 0, 0, $dimension_unit, $weight_unit, $price_unit, $currency, $steelgrade_id, $thickness, $width, $length, $unitweight, $qtty, $width, $price, $value, '', '', $internal_notes);
     ...
     * @return resource
     */
    function Save($id, $order_for, $biz_id, $company_id, $person_id, $buyer_ref, $supplier_ref, $delivery_point, $delivery_town, $delivery_cost, $delivery_date, $delivery_date_alt, $invoicingtype_id, $paymenttype_id, $status, $description, $price_equivalent) {
        ...
        $result = $this->CallStoredProcedure('sp_order_save', array($this->user_id, $id, $order_for, $biz_id,
            ...
            $dimension_unit, $weight_unit, $price_unit, $currency, $description, $price_equivalent));
    ...
    function FillOrderInfo($rowset, $id_fieldname = 'order_id', $entityname = 'order', $cache_prefix = 'order', $fill_from_session = false) {
        ...
        foreach ($rowset as $key => $row) {
            ...
            if (isset($row[$entityname]) && !empty($row[$entityname])) {
                ...
                if ($order['biz_id'] > 0)
                    $rowset[$key]['orderbiz_id'] = $order['biz_id'];
                if (!empty($order['company_id']))
                    $rowset[$key]['ordercompany_id'] = $order['company_id'];
                if (!empty($order['person_id']))
                    $rowset[$key]['orderperson_id'] = $order['person_id'];
    ...
    function _get_interval($value) {
        ...
        if (!empty($matches))
            return array('from' => floatval($matches[1]), 'to' => floatval($matches[1]));

        // 0.65-0.89
        preg_match("#^([0-9\.]+)-([0-9\.]+)$#si", $value, $matches);
        if (!empty($matches))
            return array('from' => floatval($matches[1]), 'to' => floatval($matches[2]));

        // >0.89
        preg_match("#^&gt;([0-9\.]+)$#si", $value, $matches);
        if (!empty($matches))
            return array('from' => floatval($matches[1]), 'to' => 0);

        // <0.89
        preg_match("#^&lt;([0-9\.]+)$#si", $value, $matches);
        if (!empty($matches))
            return array('from' => 0, 'to' => floatval($matches[1]));
    ...
    public function GetListOfRelatedDocs($id) {
        ...
        foreach ($rowset as $doc_set) {
            ...
            foreach ($docs_list as $key => $set) {
                if (!array_key_exists($object_alias, $set))
                    continue;
    ...
    /**
     * изменяет Price Equivalent в заказе
     *
     * @version 20140616
     * @author Uskov
     */
    function ChangePrice($order_id, $price) {
        $result = $this->Update($order_id, array('price_equivalent' => $price));
    ...
    public function getCompliteOrders($arg) {
        //dg($arg);
        if (empty($stock_id)) {
            $stock_id = 1;
        }
        ...
        $str_where = '';
        if ($arg['stock_id'] > 0)
            $str_where .= ' AND orders.stock_id="' . $arg['stock_id'] . '"';
        if (!empty($arg['date_end']))
            $str_where .= ' AND orders.modified_at <= "' . $arg['date_end'] . '"';
        if (!empty($arg['date_start']))
            $str_where .= ' AND orders.modified_at >= "' . $arg['date_start'] . '"';
        if ($arg['customer_id'] > 0)
            $str_where .= ' AND orders.company_id IN (' . $arg['customer_id'] . ')';
        if ($arg['delivery_town'] !== '')
            $str_where .= ' AND orders.delivery_town LIKE "%' . $arg['delivery_town'] . '%"';
        if ($arg['steelgrade_ids'] > 0 )
            $steelgrade_where = " AND order_positions.steelgrade_id = '{$arg['steelgrade_ids']}' ";
        if ($arg['location_ids'] > 0 )
            $location_ids = " AND steelitems.location_id = '{$arg['location_ids']}' ";
        if ($arg['thickness'] > 0)
            $thickness = "AND thickness = '{$arg['thickness']}' ";
        if ($arg['width'] > 0)
            $width = " AND width = ' {$arg['width']}' ";
        if ($arg['lenght'] > 0)
            $lenght = " AND lenght = '{$arg['lenght']}' ";
        if ($arg['weight'] > 0)
            $weight = " AND weight = ' {$arg['weight']}' ";
        if ($arg['thickness_mm'] > 0)
            $thickness_mm = "AND thickness_mm = '{$arg['thickness_mm']}' ";
        if ($arg['width_mm'] > 0)
            $width_mm = " AND width = ' {$arg['width_mm']}' ";
        if ($arg['lenght_mm'] > 0)
            $lenght_mm = " AND lenght = '{$arg['lenght_mm']}' ";
        if ($arg['weight_ton'] > 0)
            $weight_mm = " AND weight = ' {$arg['weight_mm']}' ";
        ...
        $arg_query = array(
            ...
            'join' => array(
                array(
                    ...
                    'type' => 'inner',
                    'conditions' => "order_positions.order_id = orders.id $steelgrade_where "
                    . $thickness.  $width.  $lenght.  $weight
                    . $thickness_mm.  $width_mm. $lenght_mm. $weight_mm
                ),
                array(
                    'table' => 'steelitems',
                    'conditions' => "steelitems.steelposition_id = order_positions.position_id  $location_ids "
                ),
            ),
            'where' => 'orders.status = "co" ' . $str_where,
    ...
    public function getDeliveryPoints($title) {
        if (!isset($title)) {
            return false;
        }
        $arg_query = array(
            'fields' => array('DISTINCT delivery_town AS title'),
            'where' => "status = 'co' AND delivery_town LIKE '%{$title}%'"
            , 'order' => 'id DESC'
        );

        $rowset = $this->table->SelectList($arg_query);
        foreach ($rowset as $row) {
            $deliverypoints[] = $row['title'];
        }
        return $deliverypoints;
    }

    public function getCustomers($title, $order_where) {
        $arg_query = array(
            'fields' => array('DISTINCT companies.title AS title, orders.company_id AS id'),
            'join' => array(
                array(
                    'table' => 'companies',
                    'type' => 'INNER',
                    'conditions' => "orders.company_id = companies.id AND companies.title LIKE '%{$title}%' ",
                ),
            ),
            'where' => "orders.status = 'co' " . $order_where,
            'order' => 'orders.id DESC'
        );
