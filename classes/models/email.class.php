...
class Email extends Model {
    ...
    public function DeleteSpam($email_id) {
        $result = $this->CallStoredProcedure('sp_email_delete', array($this->user_id, $email_id));
...
//TODO: реализовать фильтрацию по doc_type_id
    ...
    public function GetAjaxTable($object_id, $type_id, $is_deleted, $keyword, $page_no = 0, $per_page = ITEMS_PER_PAGE) {
        //dg($keyword);
        if (!empty($keyword)) {
        ...
        //параметры поиска необходимо передавать в виде массивов
        if(count($type_id) > 0) $cl->SetFilter('type_id', $type_id);
        if(count($object_id) > 0) $cl->SetFilter('object_id', $object_id);
        if(count($mailbox_id) > 0) $cl->SetFilter('mailbox_id', $mailbox_id);
        //$cl->SetFilter('object_alias_id', array(sprintf("%u", crc32('biz') & 0xffffffff)));
    ...
    function _search($object_alias, $object_id, $mailbox_id, $type_id, $keyword, $hash, $start, $per_page) {
        ...
        if (!isset($rowset) || !isset($rowset['data']) || isset($rowset['outdated'])) {
            ...
            if (!empty($keyword)) {
                if ($type_id > 0)
                    $cl->SetFilter('type_id', array($type_id, EMAIL_TYPE_SPAM));
                //dg(array($type_id, EMAIL_TYPE_SPAM));
            }else {
                if ($type_id > 0)
                    $cl->SetFilter('type_id', array($type_id));
                //dg(array($type_id));
            }
            ...
            if (empty($object_alias) || empty($object_id)) {
                ...
                if ($this->user_role > ROLE_ADMIN) {
                    if ($mailbox_id > 0 && in_array($mailbox_id, $user_mailboxes)) {
                        ...
                        if (empty($user_mailboxes))
                            $user_mailboxes = array(0);
                ...
                if (empty($type_id))
                    $cl->SetFilter('type_id', array(EMAIL_TYPE_INBOX, EMAIL_TYPE_OUTBOX, EMAIL_TYPE_SPAM));
                //dg($cl);
    ...
    function SaveObjects($email_id, $object_alias, $object_id) {
        if (empty($object_alias) || $object_alias == 'email' || empty($object_id))
            return;
        ...
        if ($object_alias == 'qc') {
            ...
            if (!empty($qc['order_id']))
                $objects[] = array('alias' => 'order', 'id' => $qc['order_id']);
            if (!empty($qc['biz_id']))
                $objects[] = array('alias' => 'biz', 'id' => $qc['biz_id']);
            if (!empty($qc['customer_id']))
                $objects[] = array('alias' => 'company', 'id' => $qc['customer_id']);
        }
...
//    }
    ...
    function SaveObject($email_id, $object_alias, $object_id) {
        ...
        if (isset($result) && array_key_exists('ErrorCode', $result))
            return null;
    ...
    function Save($id, $object_alias, $object_id, $sender_email, $recipient_email, $cc_email, $bcc_email, $to, $attention, $subject, $our_ref, 
                    ...
                    $parent_id = 0, $signature2 = '', $signature3 = '', $driver, $navigator)
    {
        ...
        $result = $this->CallStoredProcedure('sp_email_save', array($this->user_id, $id, $object_alias, $object_id, $sender_email, 
                ...
                $approve_by, $approve_deadline, $doc_type, $seek_response, $mailbox_id, $parent_id, $signature2, $signature3, $driver, $navigator));
        $result = isset($result) && isset($result[0]) && isset($result[0][0]) ? $result[0][0] : null;
    ...
    function FillEmailInfo($rowset, $id_fieldname = 'email_id', $entityname = 'email', $cache_prefix = 'email') {
        ...
        foreach ($rowset as $key => $row) {
            ...
            if (!empty($row['parent_id']))
                $rowset[$key]['eparent_id'] = $row['parent_id'];
        }
            ...
            if (isset($row[$entityname]['objects'])) {
                ...
                foreach ($biz_tags as $biz_item) {
                    ...
                    for ($i = 0; $i < count($navigators); $i++) {
                        if ($navigators[$i] > 0)
                            $navigator[] = $personModel->GetById($navigators[$i]);
                        //print_r($navigator[$i]);
                    }
                }
        ...
        foreach ($rowset as $key => $row) {
            if (isset($row[$entityname]) && isset($row[$entityname]['attachments']) && !empty($row[$entityname]['attachments'])) {
                ...
                foreach ($row[$entityname]['attachments'] as $attachment) {
                    if (!array_key_exists('attachment', $attachment))
                        continue;
...
//            'limit' => array('lower' => 0, 'number' => 1),
     ...
     * @return array
     */
    public function SaveEmailRaw($username, $mailbox_name, $message_num, $message_id, $sender_email, $recipient_email, $cc, $bcc, $reply_to, $sender, $date, $date_mail, $udate, $subject, $text_plain, $text_html, $size_plain, $size_html, $recent, $unseen, $flagged, $answered, $deleted, $draft, $is_parsed, $has_attachments, $status_id) {
...
// START EMAIL_DELIVERED methods
    ...
    public function DeleteByUser($email_id, $object_alias = '', $object_id = 0) {
        return $this->CallStoredProcedure('sp_email_delete_by_user', array($this->user_id, $email_id));
...
//        $attachments        = $modelAttachment->FillAttachmentInfo($data_set);
    ...
    public function GetDocTypeForMenu($object_alias, $object_id) {
        ...
        if (!isset($rowset['data']) || empty($rowset['data']))
            return array();
    ...
    public function GetHistory($email_id) {
        ...
        if (!isset($result) || (isset($result[0]) && isset($result[0]['ErrorCode'])))
            return null;
     ...
     * Отличаются от старых тем, что позволяют передавать несколько значений
     */
    
   /**
     * New.Возвращает список писем
     * 
     * @param mixed $object_alias
     * @param mixed $object_id
     * @param mixed $mailbox_id
     ...
     * @version 20141021, SG
     */
    //                          ($object_alias, $object_id, $mailbox_id, $type_id, $is_deleted, $keyword, $approve_by, $start, $this->limit)
    public function NewGetList($object_alias, $object_id, $mailbox_id, $type_id, $is_deleted, $keyword, $approve_by, $start = 0, $limit = ITEMS_PER_PAGE) {
        //формируем хеш для кеширования результатов
        $hash = 'emails-filter-' . md5('object_alias-' . $object_alias . '-object_id-' . $object_id . '-mailbox-' . $mailbox_id .
                        '-type-' . $type_id . '-doctypeid-' . $doc_type_id . '-is_deleted-' . $is_deleted . '-keyword-' . $keyword . '-approve_by-' . $approve_by . '-start-' . $start . '-limit-' . $limit);
        
        if (empty($keyword) || $keyword == "") { 
            return $this->_new_get_list($object_alias, $object_id, $mailbox_id, $type_id, $is_deleted, $approve_by, $hash, $start, $limit);
        } else {
        //TODO: реализовать фильтрацию по is_deleted
        //TODO: реализовать фильтрацию по approve_by
        //TODO: реализовать фильтрацию по doc_type_id

            return $this->_new_search($object_alias, $object_id, $mailbox_id, $type_id, $keyword, $hash, $start, $limit);
        }
    }    

    /**
     * New. Возвращает список писем по параметрам
     * 
     * @param mixed $object_alias
     * @param mixed $object_id
     * @param mixed $mailbox_id
     ...
     * @version 20141021, SG
     */
    function _new_get_list($object_alias, $object_id, $mailbox_id, $type_id, $is_deleted, $approve_by, $hash, $start, $limit) {

        $is_admin = ($this->user_role <= ROLE_ADMIN ? 1 : 0);
        $cache_tags = array($hash, 'emails', 'emails-type-' . $type_id, 'emails-mailbox-' . $mailbox_id .'-biz-id-'.$object_id. '-type-' . $type_id . '-start-' . $start . '-limit-' . $limit);
            //dg($type_id);
        $rowset = $this->_get_cached_data($hash, 'sp_email_get_list_new', array($this->user_id, $is_admin,
            $object_alias, $object_id, $mailbox_id, $type_id, $is_deleted, $approve_by, $start, $limit), $cache_tags);
        //dg($rowset);
        return array(
            'data' => isset($rowset[0]) ? $this->FillEmailInfo($rowset[0]) : array(),
            'count' => isset($rowset[1]) && isset($rowset[1][0]) && isset($rowset[1][0]['rows']) ? $rowset[1][0]['rows'] : 0
     ...
     * @version 20120918, zharkov
     */
    function _new_search($object_alias, $object_id, $mailbox_id, $type_id, $keyword, $hash, $start, $limit) {
        //print_r($type_id);
        $rowset = Cache::GetData($hash);
        //$rowset = null; // test mode
        if($type_id !== '') $type_id_arr = explode(', ', $type_id);
        if($mailbox_id !== '') $mailbox_id_arr = explode(', ', $mailbox_id);
        if($object_id !== '')$object_id_arr = explode(', ', $object_id);
        if (!isset($rowset) || !isset($rowset['data']) || isset($rowset['outdated'])) {
            if (!empty($keyword))
                $keyword = '*' . str_replace('-', '\-', str_replace(' ', '* *', $keyword)) . '*';
            ...
            if (!empty($keyword)) {
                if (count($type_id_arr) > 0) $cl->SetFilter('type_id', $type_id_arr);
                //dg(array($type_id, EMAIL_TYPE_SPAM));
            }else {
                if (count($type_id_arr) > 0)
                    $cl->SetFilter('type_id', $type_id_arr);
                //dg(array($type_id));
            }
            ...
            if (count($object_id_arr)<=0 || $object_id == '') {
                // если простому пользователю доступен к просмотру заданный ящик, то он показывается, если нет ,то показываются все доступные ящики
                if ($this->user_role > ROLE_ADMIN) {
                    if (count($mailbox_id_arr)>0) {
                        //$cl->SetFilter('mailbox_id', $mailbox_id_arr);
                    } else {
                ...
                if (count($type_id_arr) <= 0) $cl->SetFilter('type_id', array(EMAIL_TYPE_INBOX, EMAIL_TYPE_OUTBOX, EMAIL_TYPE_SPAM));
                //dg($cl);
                $indexes = 'ix_mam_emails, ix_mam_emails_delta';
            }
            else {
                $cl->SetFilter('object_alias_id', array(sprintf("%u", crc32($object_alias) & 0xffffffff)));
                $cl->SetFilter('object_id', $object_id_arr);
            ...
            Cache::SetData($hash, $rowset, array('emails', 'search', 'emails-type-' . $type_id . '-biz-'.$object_id), CACHE_LIFETIME_MIN);
