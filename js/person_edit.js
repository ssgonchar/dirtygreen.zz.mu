...
//Добовляет контакт
var add_contac = function()
{
    $('.tpl-row').clone().appendTo("#cd-list").removeClass('tpl-row').addClass('contact_row').show();
    $(".contact_row:last select").addClass('chosen-select').chosen();
};
 //Удаляет контакт
 var remove_contac = function(obj)
{
    if (!confirm('Remove contact data ?')) return;    
    $(obj).parent().remove();
};
//Добавляем редактор тини к заметкам в персонале
tinymce.init({
  selector : '#notes',
  plugins: [
