...
$(function(){    
    /*
     * обработчик клика по ячейке Price Equivalent
     */
    $('.order-price-td').live("click", function(event){        
        event.preventDefault();
        event.stopPropagation();
        
        var val = $(this).text();
        var obj = $(this);
        var html= "<input type='text' class='order-price-edit' value='"+val+"'>";
        
        obj.html(html);
        console.log(val);
    });
    
    $('.order-price-edit').live("click", function(event){
        event.preventDefault();
        event.stopPropagation();        
    });
    //при вводе значения в Price Equivalent сохраняем
    $('.order-price-edit').live("keyup", function(event){
 ...
 * Uskov
 */
var save_order_price = function(id, price) {
    $.ajax ({
        url: '/order/saveprice',
        data : {
            order_id         : id,
        ...
        {
            console.log("Order price was successfully updated, id: " + json.object);
        }
    });    
};
