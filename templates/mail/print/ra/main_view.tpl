        ...
        <td class="text-top">
            <table class="form" style="width: 100%;">
                ...
                <tr style="height: 32px;">
                    ...
                    <td>{if $ra.weighed_weight > 0}{$ra.weighed_weight|string_format:'%.3f'} {$ra.weight_unit|wunit}{else}{''|undef}{/if}</td>
                </tr>                
                <tr style="height: 32px;">
                    <td class="form-td-title-b">Max Weight : </td>
                    {if !empty($ra.total_weight_max)}<td{if !empty($ra.total_weightmax_highlight)} class="ra-td-bgcolor-yellow"{/if}>{$ra.total_weight_max|string_format:'%.3f'} {$ra.weight_unit|wunit}</td>
                    {else}<td>{''|undef}</td>
                    {/if}
                </tr>
                <tr style="height: 32px;">
                    ...
                    <td>
                        {if isset($ra.attachment)}<a class="pdf" target="_blank" href="/file/{$ra.attachment.secret_name}/{$ra.attachment.original_name}">{$ra.attachment.original_name}</a>{else}{''|undef}{/if}
                        {if !empty($objects_list)}
                            {foreach $objects_list as $row}
                            {if $row.is_outdated == 0 && isset($row.attachment)}
                            <a class="pdf" target="_blank" href="/file/{$row.attachment.secret_name}/{$row.attachment.original_name}">{$row.attachment.original_name}</a>
                            {/if}
                            {/foreach}
                        {/if}
                    </td>
                ...
                <tr style="height: 32px;">
                    <td class="form-td-title-b text-top">DDTs / CMRs : </td>
                    <td class="text-top">
                    {if empty($objects_list)}
                        {''|undef}
                    {else}
                        {foreach $objects_list as $row}
                            <a class="tag-document" style="display: inline-block; height: 20px;" href="/{$row.object_alias}/{$row.object_id}" target="_blank">{$row.doc_no|escape:'html'|undef}</a>
                        {/foreach}
                    {/if}
                    </td>
                </tr>
...
<table class="list" style="width: 100%">
    <tr class="top-table" style="height: 25px;">
        ...
        <th rowspan="2">Thickness,<br />{$ra.dimension_unit}</th>
        <th rowspan="2">Width,<br />{$ra.dimension_unit}</th>
        <th rowspan="2">Length,<br />{$ra.dimension_unit}</th>
        <th rowspan="2">Weight,<br />{$ra.weight_unit}</th>
...
<table class="form" width="100%">
    <tr>
        <td width="33%" class="text-top">
            <table class="form" style="width: 100%;">
                <tr height="32">
                    <td class="form-td-title-b" style="width:170px;">Created : </td>
                    <td>{$ra.created_at|date_human}, {$ra.author.login}</td>
                </tr>
                <tr height="32">
                    <td class="form-td-title-b">Modified : </td>
                    <td>{$ra.modified_at|date_human}, {$ra.modifier.login}</td>
                </tr>
            </table>
        </td>
    </tr>
