...
*}                
            ...
            <tr id="item-{$row.steelitem.id}"{if $row.steelitem.status_id >= $smarty.const.ITEM_STATUS_ORDERED} class="item-status-{$row.steelitem.status_id}"{/if}>
                ...
                <td onclick="show_item_context(event, {$row.steelitem_id});" class="text-center" id="item-weight-{$row.steelitem_id}">{$row.steelitem.unitweight|escape:'html'|string_format:'%.2f'}{if $item_weight_unit_count > 1} {$row.steelitem.weight_unit|wunit}{/if}</td>
