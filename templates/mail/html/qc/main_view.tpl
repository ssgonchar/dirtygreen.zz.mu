    ...
    <tr>
        <td width="50%" class="text-top">
            <table class="form" width="100%">                
                ...
                {/if}
                {if !empty($qc.test_ref)}
                <tr>
                    <td class="text-right" style="font-weight: bold;">Test Ref. : </td>
                    <td>{$qc.test_ref|escape:'html'}</td>
                </tr>
                {/if}
                {*<tr>
...
<table id="t-i" class="list" width="100%">
    <tbody>
        <tr class="top-table">
            ...
            <th>Steel Grade</th>
            <th>Thickness{if !isset($multiunits)}, {$qc.dim_unit}{/if}</th>
            <th>Width{if !isset($multiunits)}, {$qc.dim_unit}{/if}</th>
            <th>Length{if !isset($multiunits)}, {$qc.dim_unit}{/if}</th>
            <th>Pcs</th>
            <th>Weight{if !isset($multiunits)}, {$qc.wght_unit|wunit}{/if}</th>
            <th>Heat / Lot</th>
...
<div class="pad"></div>
{include file='templates/controls/object_shared_files.tpl' object_alias='qc' object_id=$qc.id}