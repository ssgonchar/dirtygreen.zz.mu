<div class="chat-user-container"{if isset($readonly) && $readonly} style="cursor:auto;"{/if}>
...
{if isset($users.partners) && !empty($users.partners)}
    ...
    <div class="chat-user-container tooltip" alt="{$user_row.user.person.doc_no}" title="{$user_row.user.person.doc_no} || {$user_row.user.person.company.doc_no}"{if isset($readonly) && $readonly} style="cursor:auto;"{/if}>
        {include file='templates/html/chat/control_recipient.tpl' recipient=$user_row}
    </div>
    {/foreach}
{/if}