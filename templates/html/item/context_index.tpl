	...
	<ul class="nav navbar-nav navbar-right">
		<li style='margin-top: 6px;'>
			{if !isset($is_revision) || empty($is_revision)}
				{if isset($target_doc)}
					<input type="button" class="btn btn-default" value="{$back_title}" onclick="location.href='/{if $target_doc == 'supinvoice'}supplierinvoice{else}{$target_doc}{/if}/{if empty($target_doc_id)}add{else}{$target_doc_id}/edit{/if}';" style="margin: 7px;">
