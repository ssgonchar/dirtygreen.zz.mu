...
<td>{if isset($row.email.userdata) && $row.email.userdata.read_at > 0}{else}<b>(new) <b>{/if}{$row.email.title}</td>
            ...
            <td>
                {if $row.email.type_id == 1 || $row.email.type_id == 5}<a class="btn btn-sm btn-primary" onclick="window.open('/emailmanager/{$row.email.id}', 'email_html_{$row.email.id}', 'fullscreen=yes,scrollbars=yes,resizable=yes');"><i class="glyphicon glyphicon-eye-open"></i></a>{/if}
