...
{else}
    <table class="list search-target" width="100%">
        ...
        <tr class="{if !empty($row.ra.ddt_number)}ra-row-with-ddt{/if}">
            ...
            <td{if $row.ra.status_id == $smarty.const.RA_STATUS_OPEN && !empty($row.ra.total_weightmax_highlight)} style="background-color: #FFF750 !important;"{/if} onclick="location='/ra/{$row.ra.id}';">
                {if $row.ra.weight_unit == 'lb'}
                    {$row.ra.total_weight|number_format:0:true} {$row.ra.weight_unit}
                {else}
                    {$row.ra.total_weight|number_format:2:true} ton
                {/if} ({$row.ra.total_qtty} pcs)
            </td>
            {* weighed weight *}
            <td onclick="location='/ra/{$row.ra.id}';">
                {if $row.ra.weighed_weight > 0}
                    {if $row.ra.weight_unit == 'lb'}
                        {$row.ra.weighed_weight|number_format:0:true} {$row.ra.weight_unit}
                    {else}
                        {$row.ra.weighed_weight|number_format:3:true} ton
                    {/if}
                {else}
                    {''|undef}
                {/if}
