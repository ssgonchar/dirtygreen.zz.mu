...
<div id="position-add-table"{if !isset($stock)} style="display: none;"{/if}>
    <span>Steel Grade, Thickness, Width, Length, Unit Weight, Qtty, Weight, Price, Delivery time, Value are required .</span>
    ...
    <table class="list" width="100%">
        <tbody>
            ...
            <tr>
                ...
                <td><input type="text" name="positions[{$smarty.foreach.pos.index}][thickness]"{if !empty($position.thickness)} value="{$position.thickness|escape:'html'}"{/if} id="thickness-{$smarty.foreach.pos.index}" class="max thickness-input" onkeyup="addpos_calc_unitweight({$smarty.foreach.pos.index});"></td>
                <td><input type="text" name="positions[{$smarty.foreach.pos.index}][width]"{if !empty($position.width)} value="{$position.width|escape:'html'}"{/if} id="width-{$smarty.foreach.pos.index}" class="max width-input" onkeyup="addpos_calc_unitweight({$smarty.foreach.pos.index});"></td>
                <td><input type="text" name="positions[{$smarty.foreach.pos.index}][length]"{if !empty($position.length)} value="{$position.length|escape:'html'}"{/if} id="length-{$smarty.foreach.pos.index}" class="max length-input" onkeyup="addpos_calc_unitweight({$smarty.foreach.pos.index});"></td>
                <td><input type="text" name="positions[{$smarty.foreach.pos.index}][unitweight]"{if !empty($position.unitweight)} value="{$position.unitweight|escape:'html'}"{/if} id="unitweight-{$smarty.foreach.pos.index}" class="max unitweight-input" onkeyup="addpos_calc_weight({$smarty.foreach.pos.index});"></td>
                <td><input type="text" name="positions[{$smarty.foreach.pos.index}][qtty]"{if !empty($position.qtty)} value="{$position.qtty|escape:'html'|string_format:'%d'}"{/if} id="qtty-{$smarty.foreach.pos.index}" class="max qtty-input" onkeyup="addpos_calc_weight({$smarty.foreach.pos.index});"></td>
                <td><input type="text" name="positions[{$smarty.foreach.pos.index}][weight]"{if !empty($position.weight)} value="{$position.weight|escape:'html'|string_format:'%.2f'}"{/if} id="weight-{$smarty.foreach.pos.index}" class="max weight-input" onkeyup="addpos_calc_value({$smarty.foreach.pos.index});"></td>
