<li {if isset($itemid)}id="{$itemid}" {/if}class="chat-message" style="color: {if $message.message.type_id == $smarty.const.MESSAGE_TYPE_ORDER}red{else}{$message.message.sender.color}{/if};">
    <div>
        ...
        <div style="float: left; width: 70px; font-size: 11px; line-height: 14px;{if $message.message.type_id == $smarty.const.MESSAGE_TYPE_LOGIN_FAILED} color: red;{/if}">
        ...
        <div class="chat-message-subject" style="float: left; width: 600px; line-height: 14px;">
        {if $message.message.type_id == $smarty.const.MESSAGE_TYPE_NORMAL || $message.message.type_id == $smarty.const.MESSAGE_TYPE_PRIVATE || $message.message.type_id == $smarty.const.MESSAGE_TYPE_PERSONAL}
            ...
            <br><b>{$message.message.title|biz_format:'blog'}</b>
        ...
        {elseif $message.message.type_id == $smarty.const.MESSAGE_TYPE_LOGIN_FAILED}
            ...
            <br><b>{$message.message.title|biz_format:'blog'}</b></span>
        ...
        {elseif $message.message.type_id == $smarty.const.MESSAGE_TYPE_ORDER}
            <b>{$message.message.title|biz_format:'blog'}</b>
        {/if}            
        </div>
        <div style="float: right;">            
            {if $smarty.session.user.role_id <= $smarty.const.ROLE_STAFF && !empty($message.message.is_pending) && isset($message.message.is_pending_recipient) && (empty($message.message.userdata) || empty($message.message.userdata.done_at))}<a id="message-{$message.message.id}-pending"class="pending-black" onclick="mark_message_as_done({$message.message.id});">{if !empty($message.message.deadline)}&nbsp;Deadline : {$message.message.deadline|date_format:'d/m/Y'}{/if}</a>{/if}
        </div>            
        <div class="separator"></div>
    </div>
    <div class="chat-message-text">
        {if $message.message.type_id == $smarty.const.MESSAGE_TYPE_LOGIN_FAILED}
        <span style="color:red;">{$message.message.description}</span>
        {else}
        {$message.message.description}</b></i></a>{* trick for closing unclosed <b>, <i> & <a> tags*}
        {/if}
    </div>
