<table class="form" width="50%">
    <tr>
        <td><input type="text" name="form[keyword]" class="form-control"{if isset($keyword)} value="{$keyword|escape:'html'}"{/if}></td>
...
<ol class="sr-items search-target">
    ...
    <li class="sr-item">
        ...
        <div class="sr-item-pic">                    
            <a href="/person/{$row.person.id}{if $smarty.session.user.role_id==2 || $smarty.session.user.role_id==4}/edit{/if}">
        ...
        <div class="sr-item-data">
            <div class="sr-item-title">
                <h2><a href="/person/{$row.person.id}{if isset($row.person.id)}/edit{/if}">{$row.person.full_name}</a></h2>
            </div>
