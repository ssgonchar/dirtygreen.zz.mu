$( document ).ready(function(){
   ...
   /*
    ...
    $('.make-ref').live('hidden.bs.popover', function(){
        $(this).show();
    }); 
    
    $('.make-ref').live('click', function(event){
        event.stopPropagation();
        $('.make-ref').css('display','inline-block');
        //$('.make-ref').popover('hide');
        //$('.popover').hide();
        $(this).popover('show');
    });*/
$('.make-ref').live('click', function(event){
...
$('.make-ref').live('hover', function(event){
    ...
    //$('.ui-datepicker-calendar').css('font-size', '50%');
    //$('.ui-datepicker').css('width', 'auto');
    /*$( ".chat-archive-dateto" ).datepicker( "option", "width", '100%');*/
    //$( ".ui-datepicker-inline" ).css( "width", '100%');
        //$("#chat-toolbox").height(($(window).height()-200)+'px');
        //$("#grid").jqGrid('setGridHeight', $(window).height()-200, true);
	...
	get_pending_counter();
            ...
            //$('.col2').width('250px');
            //console.log($('.col2').width());
            $('input').tooltip();
            
            //$('.timeline-badge img').hide();
            //$('.timeline-badge').hide();
            $('.timeline-badge img').addClass('img-circle');
            $('.timeline-badge img').css('margin-top', '0px');
