...
<table id="positions" class="list" width="100%"><tbody>
    ...
    <tr>
        ...
        <td{if $order.status != 'ca'} onclick="location.href='/order/selectitems/{$order.id}/position:{$row.position_id}';"{/if}>{if isset($row.length)}{$row.length|escape:'html'}{/if}</td>
        <td{if $order.status != 'ca'} onclick="location.href='/order/selectitems/{$order.id}/position:{$row.position_id}';"{/if}>{if isset($row.unitweight)}{$row.unitweight|escape:'html'|string_format:'%.2f'}{/if}</td>
        ...
        <td{if $order.status != 'ca'} onclick="location.href='/order/selectitems/{$order.id}/position:{$row.position_id}';"{/if}>{if isset($row.weight)}{$row.weight|escape:'html'|string_format:'%.2f'}{/if}</td>
        ...
        {if $order.status == 'ip'}
        <td{if $order.status != 'ca'} onclick="location.href='/order/selectitems/{$order.id}/position:{$row.position_id}';"{/if}>{if $row.balance_to_deliver.qtty > 0}{$row.balance_to_deliver.weight|string_format:'%.2f'} ({$row.balance_to_deliver.qtty}){else}<span style="color: #999;">delivered</span>{/if}</td>
        {/if}
