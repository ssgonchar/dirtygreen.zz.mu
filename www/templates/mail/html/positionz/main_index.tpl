...
<table width="100%" class="form">
    <tr>
        ...
        <td>
            <input type="text" value="{if isset($thickness) && $thickness > 0}{$thickness}{/if}" name="form[thickness]" style="width: 100%;">
        </td>
        <td class="text-right" width="120px">Width exact :</td>
        <td>
            <input type="text" value="{if isset($width) && $width > 0}{$width}{/if}" style="width: 100%;" name="form[width]"> 
        </td>
        <td class="text-right" width="120px">Length exact :</td>
        <td>
            <input type="text" value="{if isset($length) && $length > 0}{$length}{/if}" style="width: 100%;" name="form[length]">
        </td>  
        <td class="text-right" width="120px">Weight exact :</td>
        <td>
            <input type="text" value="{if isset($weight) && $weight > 0}{$weight}{/if}" style="width: 100%;" name="form[weight]">
        </td>     
    </tr>
    <tr>
        <td></td>
        ...
        <td>
            <input type="text" value="{if isset($thicknessmin) && $thicknessmin > 0}{$thicknessmin}{/if}" style="width: 100%;" name="form[thicknessmin]">
        </td>
        <td class="text-right">min :</td>
        <td>
            <input type="text" value="{if isset($widthmin) && $widthmin > 0}{$widthmin}{/if}" style="width: 100%;" name="form[widthmin]">
        </td>
        <td class="text-right">min :</td>
        <td>
            <input type="text" value="{if isset($lengthmin) && $lengthmin > 0}{$lengthmin}{/if}" style="width: 100%;" name="form[lengthmin]">
        </td>
        <td class="text-right">min :</td>
        <td>
            <input type="text" value="{if isset($weightmin) && $weightmin > 0}{$weightmin}{/if}" style="width: 100%;" name="form[weightmin]">
        </td>
    </tr>
    <tr>
        <td class="text-right">Keyword @ Notes :</td>
        <td>
            <input type="text" value="{if isset($keyword) && !empty($keyword)}{$keyword|escape:'html'}{/if}" style="width: 100%;" name="form[keyword]">
        </td>
        <td class="text-right">max :</td>
        <td>
            <input type="text" value="{if isset($thicknessmax) && $thicknessmax > 0}{$thicknessmax}{/if}" style="width: 100%;" name="form[thicknessmax]">
        </td>
        <td class="text-right">max :</td>
        <td>
            <input type="text" value="{if isset($widthmax) && $widthmax > 0}{$widthmax}{/if}" style="width: 100%;" name="form[widthmax]">
        </td>
        <td class="text-right">max :</td>
        <td>
            <input type="text" value="{if isset($lengthmax) && $lengthmax > 0}{$lengthmax}{/if}" style="width: 100%;" name="form[lengthmax]">
        </td>    
        <td class="text-right">max :</td>
        <td>
            <input type="text" value="{if isset($weightmax) && $weightmax > 0}{$weightmax}{/if}" style="width: 100%;" name="form[weightmax]">
        </td>
    </tr>
</table>    
{*
...
<table class="list" width="100%">
    <tbody>
        <tr class="top-table">
            <th width="3%"><input class="chb" type="checkbox" disabled="disabled" onchange="check_all(this, 'position'); calc_selected(); show_group_actions();" style="margin-left: 2px;"></th>
        ...
        <tr id="position-{$row.steelposition_id}">
            ...
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if} class="pos">{$row.steelposition.steelgrade.title}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{$row.steelposition.thickness|escape:'html'}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{$row.steelposition.width|escape:'html'}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{$row.steelposition.length|escape:'html'}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});" id="position-unitweight-{$row.steelposition_id}"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{$row.steelposition.unitweight|escape:'html'|string_format:'%.2f'}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}><span id="position-qtty-{$row.steelposition_id}">{$row.steelposition.qtty|escape:'html'|string_format:'%d'}</span>{if isset($row.steelposition.quick) && !empty($row.steelposition.quick.reserved)} (<a href="/positions/reserved">{$row.steelposition.quick.reserved}</a>){/if}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});" id="position-weight-{$row.steelposition_id}"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{$row.steelposition.weight|escape:'html'|string_format:'%.2f'}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});" id="position-price-{$row.steelposition_id}"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{$row.steelposition.price|escape:'html'|string_format:'%.2f'}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});" id="position-value-{$row.steelposition_id}"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{$row.steelposition.value|escape:'html'|string_format:'%.2f'}</td>
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});"{if isset($row.steelposition.on_stock) && !empty($row.steelposition.on_stock)} style="background-color: #FEFEFE; font-weight: bold;"{elseif !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{if isset($row.steelposition.deliverytime)}{$row.steelposition.deliverytime.title}{/if}</td>
            ...
            <td onclick="position_click(event, {$row.steelposition_id}, {$is_revision});"{if !empty($row.steelposition.bgcolor)} style="background-color: {$row.steelposition.bgcolor};"{/if}>{if isset($row.steelposition.biz)}{$row.steelposition.biz.number_output|escape:'html'}{/if}</td>
        </tr>
        {/foreach}
    </tbody>
</table>
