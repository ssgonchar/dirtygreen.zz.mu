    ...
    <tr>
        ...
        <td>
            <select id="locations" name="stockholder_id" class="normal" onchange="itemalias_resetallpositions();">
                ...
                {if $row.company.location_id > 0}<option value="{$row.company.location_id}"{if $row.company.id == $stockholder_id} selected="selected"{/if}>{$row.company.doc_no} ({$row.company.stocklocation.title})</option>{/if}
                {/foreach}
            </select>
        </td>        
    </tr>
...
<table class="list" width="100%">
    <tbody>
        <tr class="top-table">
            ...
            <th width="7%" class="text-center">Price<span class="lbl-price">{if isset($stock)},<br>{$stock.currency_sign}/{$stock.weight_unit|wunit}{/if}</span></th>
...
*}            
            <td id='td-position-{$row.id}'>
                ...
                <a href="javascript: void(0);" id="a-position-{$row.id}" onclick="itemalias_fillpositions({$row.id});" class="find" style="display: none;">find positions</a>
