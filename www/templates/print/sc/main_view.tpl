<table width="100%">
    <tr>
        ...
        <td width="25%">
            <div class="text-right title-field">Created : </div>
            <div class="value-field">{$sc.created_at|date_human}, {$sc.author.login}</div>
        </td>
    </tr>
    <tr>
        <td>
        ...
        <td>
            <div class="text-right title-field">Modified : </div>
            <div class="value-field">{$sc.modified_at|date_human}, {$sc.modifier.login}</div>
        </td>
    </tr>
    <tr>
        <td>
...
<table id="positions" class="list" width="100%"><tbody>
    <tr class="top-table">
        ...
        <th>Unit Weight,<br><span class="lbl-wgh">{if isset($order.weight_unit)}{$order.weight_unit|escape:'html'}{/if}</span></th>
        <th>Qtty,<br>pcs</th>
        <th>Weight,<br><span class="lbl-wgh">{if isset($order.weight_unit)}{$order.weight_unit|escape:'html'}{/if}</span></th>
        <th>Price,<br><span class="lbl-price">{if isset($order.currency)}{if $order.currency == 'usd'}${elseif $order.currency == 'eur'}&euro;{else}{/if}{/if}/{if isset($order.weight_unit)}{$order.weight_unit|escape:'html'}{/if}</span></th>
