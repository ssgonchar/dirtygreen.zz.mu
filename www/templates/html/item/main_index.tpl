...
<div id="more-params" {if !isset($params)} style="display: block"{/if}>
    <table width="100%">
        <tr>
            <td width="30%" class="text-top">
                <table class="form" width="100%">
...
*}                
            ...
            <tr id="item-{$row.steelitem.id}"{if $row.steelitem.status_id >= $smarty.const.ITEM_STATUS_ORDERED} class="item-status-{$row.steelitem.status_id}"{/if}>
                ...
                <td class="td-item-checkbox">
                    ...
                    <input type="checkbox" class="cb-row-item" value="{$row.steelitem.id}" onchange="calc_selected(); show_group_actions();" style="margin-right: 5px;">
                    {/if}
                </td>
                {/if}
