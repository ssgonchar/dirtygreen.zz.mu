...
<!-- Modal -->
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="false" data-show="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <div id="person-pic" onclick="person_change_pic();" style="text-align:left; display: inline-block;">
        {if isset($person.picture)}
            {picture type="person" size="x" source=$person.picture}
        {else}
            <img src="/img/nopicture{if $person.gender == 'f'}f{/if}.gif">
        {/if}
        </div>
        ...
        <h4 class="modal-title" id="myModalLabel" style="text-align:left; display: inline-block;">
            {$person.title|capitalize}. {$person.first_name} {$person.middle_name} {$person.last_name} <span class="label label-success">online</span>
        </h4>
      </div>
    ...
    <div class="row">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="form-group col-sm-6">
                                    ...
                                    <p>
                                        <strong>{$company.title|escape:'html'}</strong>
                                    </p>
                                    <input type="email" class="form-control in-place-edit" id="exampleInputEmail1" placeholder="Enter company" style="display: none;">
                                </div>
                                <div class="form-group col-sm-6">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-6">
                                    ...
                                    <p>
                                        <strong>
                                            {if isset($row.jobposition.title)}
                                                {$row.jobposition.title|escape:'html'}
                                            {else}
                                                not selected
                                            {/if}
                                        </strong>
                                ...
                                <div class="form-group col-sm-6">
                                    ...
                                    <p>
                                        <strong>
                                            {if isset($person.key_contact) && !empty($person.key_contact) || isset($company.id)}
                                                Yes
                                            {else}
                                                No
                                            {/if}
                            ...
                            <div class="row">
                                {foreach from=$contactdata item=row name=cd}
                                    <div class="form-group col-sm-6">
                                        ...
                                        <p>
                                            <strong>{$row.title|escape:'html'}</strong>
                                        </p>
                                        <input type="email" class="form-control in-place-edit" id="exampleInputEmail1" placeholder="Enter company" style="display: none;">
                                    </div>
                                {/foreach}            
                                    ...
                                    <p>
                                        <strong>
                                            {foreach from=$countries item=row}
                                                {if isset($person) && isset($person.country_id) && $person.country_id == $row.country.id}
                                                    {$row.country.title|escape:'html'}                      
                                                {/if}
                                            {/foreach}                                
                                        </strong>
                                    </p>
                                    ...
                                    <p>
                                        <strong>
                                            {foreach from=$regions item=row}
                                                {if isset($person) && isset($person.region_id) && $person.region_id == $row.region.id}
                                                    {$row.region.title|escape:'html'}                      
                                                {/if}
                                            {/foreach}                                
                                        </strong>
                                    </p>
                                    ...
                                    <p>
                                        <strong>
                                            {foreach from=$cities item=row}
                                                {if isset($person) && isset($person.city_id) && $person.city_id == $row.city.id}
                                                    {$row.city.title|escape:'html'}                      
                                                {/if}
                                            {/foreach}                                
                                        </strong>
                                    </p>
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="form-group col-sm-4">
                                    ...
                                    <p>
                                        <strong>{$user.login|escape:'html'}</strong>
                                    </p>
                                    <input type="email" class="form-control in-place-edit" id="exampleInputEmail1" placeholder="Enter company" style="display: none;">
                                </div>
                                <div class="form-group col-sm-4">
                                ...
                                <div class="form-group col-sm-4">
                                    ...
                                    <p>
                                        <strong>
                                            {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_LIMITED_USER}
                                                Site User
                                            {/if}
                                            {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_USER}
                                                Site & WebStock User
                                            {/if}
                                            {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_STAFF}
                                                MaM Staff
                                            {/if}
                                            {if $smarty.session.user.role_id <= $smarty.const.ROLE_MODERATOR}                            
                                                {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_MODERATOR}
                                                    MaM Moderator
                                                {/if}
                                                {if $smarty.session.user.role_id <= $smarty.const.ROLE_ADMIN}                            
                                                    {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_ADMIN}
                                                        MaM Admin
                                                    {/if}
                                                {/if}
                                            {/if}                                                                    
                            ...
                            <div class="form-group col-sm-2">
                              <label for="">In TL Icon Park</label>
                              <p>
                                <strong>
                            {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_LIMITED_USER}Site User{/if}
                            {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_USER}Site & WebStock User{/if}
                            {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_STAFF}MaM Staff{/if}
                            {if $smarty.session.user.role_id <= $smarty.const.ROLE_MODERATOR}                            
                            {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_MODERATOR}MaM Moderator{/if}
                            {if $smarty.session.user.role_id <= $smarty.const.ROLE_ADMIN}                            
                                {if isset($user.role_id) && $user.role_id == $smarty.const.ROLE_ADMIN}MaM Admin{/if}
                            {/if}
                            {/if}                                                                    
                                </strong>
                              </p>
        ...
        <!--
        <form role="form">
            <div class="list-group  panel panel-default" id="accordion">
              <div href="#" class="list-group-item panel-heading">
              ...
              <div href="#" id="collapseOne" class="list-group-item panel-collapse collapse in">
                    <div class="row">
                            <div class="form-group col-sm-6">
                              <label for="">Company</label>
                              <p>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-11">
                    ...
                    <input type="text" name="person[first_name]" class=""{if isset($person.first_name)} value="{$person.first_name|escape:'html'}"{/if} placeholder="First name">
                    <input type="text" name="person[middle_name]" class=""{if isset($person.middle_name)} value="{$person.middle_name|escape:'html'}"{/if} placeholder="Middle name">
                    <input type="text" name="person[last_name]" class=""{if isset($person.last_name)} value="{$person.last_name|escape:'html'}"{/if} placeholder="Last name">
                    </nobr>
                </div>
    ...
    <div class="tab-pane fade" id="account_settings">...
            ...
            <table class="form" width="100%">
                ...
                <tr>
                    ...
                    <td><input type="text" id="picker" name="user[color]" class="normal"{if isset($user.color)} value="{$user.color}"{/if}></td>
                </tr>                
                <tr>
                ...
                <tr id="chat_icon_park"{if empty($user) || empty($user.role_id)} style="display: none;"{/if}>
                 ...
                 <tr id="chat_icon_park"{if empty($user) || empty($user.role_id)} style="display: none;"{/if}>
                    ...
                    <td><input type="checkbox" name="user[driver]" value="1"{if isset($user.driver) && !empty($user.driver)} checked="checked"{/if}></td>
                </tr>               
                {if $smarty.session.user.role_id <= $smarty.const.ROLE_ADMIN}
