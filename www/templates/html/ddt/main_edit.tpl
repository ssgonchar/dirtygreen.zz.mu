    ...
    <tr>
        <td width="50%" class="text-top">
            <table class="form" style="width: 100%;">
                ...
                {/if}
                <tr>
                    <td class="form-td-title-b">Date (INIZIO TRASPORTO) : </td>
                ...
                <tr>
                    <td class="form-td-title text-top">Buyer (FUTTURARE A) : </td>
                    <td class="text-top"><textarea name="form[buyer]" class="wide" rows="5">{if !empty($form.buyer)}{$form.buyer|escape:'html'}{/if}</textarea></td>
                </tr>
                <tr>
                    <td class="form-td-title">Destination Type: </td>
                    <td>
                        <label><input type="radio" name="form[dest_type_id]" value="1" {if empty($form.dest_type_id) || $form.dest_type_id == 1}checked="cecked"{/if}  /> FATTURARE A</label>
                        <label><input type="radio" name="form[dest_type_id]" value="2" {if !empty($form.dest_type_id) && $form.dest_type_id == 2}checked="cecked"{/if} /> DESTINATARIO</label>
                    </td>
                </tr>
                <tr>
                    <td class="form-td-title">IVA (PARTITA IVA): </td>
                    <td><input type="text" name="form[iva]" value="{$form.iva|escape:'html'}" class="wide" maxlength="20" /></td>
                </tr>
                <tr>
                    <td class="form-td-title text-top">Delivery Point (DESTINAZIONE MERCE): </td>
        ...
        <td class="text-top">
            <table class="form" style="width: 100%;">
                ...
                <tr>
                    <td class="form-td-title" style="width:170px">Transporter (VETTORE) : </td>
                    <td>
                        <input type="text" id="ddt_transporter" name="form[transporter_title]" value="{if !empty($form.transporter_title)}{$form.transporter_title|escape:'html'}{/if}" class="wide ui-autocomplete-input" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true" />
                        <input type="hidden" id="ddt_transporter_id" name="form[transporter_id]" value="{if !empty($form.transporter_id)}{$form.transporter_id}{else}0{/if}" />
                    </td>
                </tr>
                <tr>
                    <td class="form-td-title text-top">Payment Term (PAGAMENTO) : </td>
                    <td>
                        <select name="form[paymenttype_id]" class="wide">
                            {include file="templates/controls/html_element_options.tpl" list=$paymenttypes_list selected=$form.paymenttype_id}
                        </select>
                    </td>
                </tr>
                <tr>
                    <td class="form-td-title text-top">Causale Trasporto (CAUSALE PORTO) : </td>
                    <td>
                        <select name="form[causale_id]" class="wide">
                            {include file="templates/controls/html_element_options.tpl" list=$causale_list selected=$form.causale_id}
                        </select>
                    </td>
                </tr>
                <tr>
                    <td class="form-td-title text-top">Porto (PORTO) : </td>
                    <td>
                        <select name="form[porto_id]" class="wide">
                            {include file="templates/controls/html_element_options.tpl" list=$porto_list selected=$form.porto_id}
                        </select>
                    </td>
                </tr>
                <tr>
                    <td class="form-td-title">Weighed Weight (PESO NETTO) : </td>
                    <td>{$form.weighed_weight|string_format:'%.2f'}&nbsp;Ton</td>
                </tr>
            </table>
        </td>
    </tr>
