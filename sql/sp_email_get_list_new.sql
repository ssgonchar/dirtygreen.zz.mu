    ...
    THEN
        ...
        THEN
            SET @var_stmt = CONCAT("   
                ...
                CASE 
                    WHEN param_approve_by > 0 THEN CONCAT(" AND  (emails.approve_by = ", param_approve_by, " OR emails.approve_by IS NULL OR emails.approve_by <= 0)")
        ...
        ELSE
            SET @var_stmt = CONCAT("   
                ...
                IF(param_is_deleted IN (1, 0), CONCAT(" AND emails.is_deleted = ", param_is_deleted), " AND emails.is_deleted = 0"),
                CASE 
                    WHEN param_approve_by > 0 THEN CONCAT(" AND (emails.approve_by = ", param_approve_by," OR emails.approve_by <= 0 OR emails.approve_by IS NULL)")
                    WHEN param_approve_by = 0 THEN " AND (emails.approve_by <= 0 OR emails.approve_by IS NULL)"
                    ELSE ""
                END,
